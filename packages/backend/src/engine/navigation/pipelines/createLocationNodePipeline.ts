...
export async function runCreateLocationNodePipeline(
  ...
  if (shouldGenerateImage) {
    // Pass space type to ensure correct prompt fixes (ceiling rule for interior only)
    const spaceType = perspective === 'exterior' ? 'exterior' : (perspective === 'interior' ? 'interior' : 'unknown');
    const result = await generateLocationImage(apiKey, imagePrompt, { spaceType });
  ...
  // Step 4: Use LLM-generated name if available, otherwise fall back to decision name
  const finalNodeName = dna.name || nodeName;
  console.log(`📝 [PIPELINE] Node name: ${finalNodeName}${dna.name ? ' (generated by LLM)' : ' (from decision)'}`);

  // Step 5: Build complete node using shared builder
  const node = buildNode(nodeType, finalNodeName, dna, imageUrl);
