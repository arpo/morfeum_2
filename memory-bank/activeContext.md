...
### Bug Fixed
Images generated by `/DISPLAY` command now automatically display without requiring page reload.

**Root Cause:** The V2 DISPLAY completion callback wasn't dispatching the `imageGenerated` event that WorldView listens for.

**Fix Applied in `v2Commands.ts`:**
```typescript
// After loading from backend, dispatch event to trigger WorldView refresh
...
**Technical Details:**
- WorldView's `useWorldViewLogic.ts` listens for `imageGenerated`, `spawnComplete`, `editImageComplete`, `imageUpscaled` events
- When event fires, it calls `fetchViews()` which clears media cache and crossfades to new image
- V1 pipelines emit these events, but V2 was missing the dispatch
- Also removed useless `updateEntityImage()` call (was targeting wrong store)

---

## 2026-01-09 - World V2 System Implementation

### Current Status: Phases 1-4 COMPLETE, Image Auto-Load Fixed
